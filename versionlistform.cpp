...
VersionListForm::VersionListForm(Project &project, QString storagePath, XmlIO * xmlwriter, QWidget *parent) :
    QDialog(parent),
...
void VersionListForm::updateVersionsList()
{
    ui->listWidget->clear();
    QStringList slist;
    for(auto it:_project->getVersions()) slist.append(it.getName());
    ui->listWidget->addItems(slist);
    int cid=_project->getCurrentVersionId();
    for(int i=0;i<_project->getVersions().length();i++)
        if (_project->getVersions()[i].getId()==cid) {ui->listWidget->setCurrentRow(i);break;}
}
