    ...
    if (ui->lineEdit_3->text() != ""){
        if (ui->lineEdit->text() != ""){
            ...
            if(!_projects -> isEmpty()) {
                Project prj(_projects->last().getId() + 1, 1, ui->lineEdit->text(),
                            _newProjectPath,versions);
                _projects -> append(prj);
            } else {
                Project prj(1, 1, ui->lineEdit->text(), _newProjectPath, versions);
                _projects -> append(prj);
            }
            _createProjectFolder(ui->lineEdit->text(), "Version 1");
        } else {
...
void ProjectCreatingForm::_createProjectFolder(const QString& projName, const QString& verName)
{
    QDir dir;
    dir.cd(_storagePath);
    dir.mkdir(projName);
    dir.cd(_storagePath + "/" + projName);
    dir.mkdir(verName);
